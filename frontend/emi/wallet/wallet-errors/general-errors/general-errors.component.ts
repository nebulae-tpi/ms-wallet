...
import { WalletErrorsService } from '../wallet-errors.service';
...
export class GeneralErrorsComponent implements OnInit, OnDestroy {
  ...
  constructor(
    private walletErrorsService: WalletErrorsService,
  ...
  refreshTable(){
    this.subscriptions.push(
      Rx.Observable.defer(() =>
        ...
        .pipe(
          mergeMap(paginator => this.walletErrorsService.getWalletErrors$(
            paginator.pageIndex,
            paginator.pageSize,
            undefined
          )),
          ...
          map(resp => resp.data.getWalletErrors),
          mergeMap(clearingErrors => this.walletErrorsService.getWalletErrorsCount$(undefined)
          .map(count => [clearingErrors, count.data.getWalletErrorsCount]))
        )
